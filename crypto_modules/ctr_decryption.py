# CTR Decryption (matching the simple XOR-based encryption)
from .utils import DEFAULT_SEED, generate_lcg_bytes
from .ctr_encryption import BLOCK_SIZE, xor_bytes, increment_counter
...
def decrypt_blocks(data: bytes, key: bytes, nonce: bytes) -> bytes:
    plaintext = bytearray()
    counter = nonce
    for i in range(0, len(data), BLOCK_SIZE):
        block = data[i:i + BLOCK_SIZE]
        # Encrypt the counter to generate keystream (same as encryption)
...
def process_text(text: str) -> str:
    ...
    plain_padded = decrypt_blocks(cipher, key, nonce)
